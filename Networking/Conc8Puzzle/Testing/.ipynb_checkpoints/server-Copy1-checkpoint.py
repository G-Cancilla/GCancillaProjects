...
multiplayer_scores_lock = threading.Lock()
...
def write_to_csv_multiplayer(player1_name, player1_score, player2_name, player2_score):
    with open('highscoresMultiplayer.csv', 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([player1_name, player1_score, player2_name, player2_score])

def read_and_return_HS_multiplayer():
    try:
        with open('highscoresMultiplayer.csv', 'r') as file:
            reader = csv.reader(file)
            highscores = list(reader)

            # Sort the high scores based on the combined score of both players
            sorted_highscores = sorted(highscores, key=lambda x: int(x[1]) + int(x[3]))

            high_scores_string = "Multiplayer High Scores:\n"
            for rank, (player1_name, player1_score, player2_name, player2_score) in enumerate(sorted_highscores, start=1):
                high_scores_string += f"{rank}. {player1_name} & {player2_name}: {player1_score} + {player2_score} = {int(player1_score) + int(player2_score)}\n"

    except FileNotFoundError:
        high_scores_string = "No multiplayer highscores found."
    return high_scores_string
    ...
    while not done:
        ...
        if choice == '2':
            ...
            with player_count_lock:
                player_count +=1
                if player_count < 2:
                    client_socket.send("Waiting for second player to join...".encode('utf-8'))
                    multiplayer_event.clear()
                else:
                    client_socket.send("Player 2 has joined...Starting Game".encode('utf-8'))
                    multiplayer_event.set()
            multiplayer_event.wait()
            ...
            with multiplayer_scores_lock:
                score_player1 = client_socket.recv(1024).decode('utf-8')
                name_player1 = client_socket.recv(1024).decode('utf-8')
                write_to_csv_multiplayer(name_player1, score_player1)

                score_player2 = client_socket.recv(1024).decode('utf-8')
                name_player2 = client_socket.recv(1024).decode('utf-8')
                write_to_csv_multiplayer(name_player2, score_player2)

                # Now send the scores back to both players
                highscore_player1 = read_and_return_HS_multiplayer()
                highscore_player2 = read_and_return_HS_multiplayer()

                client_socket.send(highscore_player1.encode('utf-8'))
                client_socket.send(highscore_player2.encode('utf-8'))
